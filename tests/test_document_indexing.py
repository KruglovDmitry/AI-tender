...
def _fake_vl(**kwargs: Any) -> tuple[dict[str, Any] | None, int]:
    ...
    return {"catalog_name": "", "products": []}, 1
...
def test_vl_indexer_page_by_page(tmp_path: Path) -> None:
    ...
    _write_pdf(
        assets / "cat.pdf",
        ["страница один UPS-1000 " * 20, "страница два UPS-2000 " * 20],
    )
    ctx = IndexingContext(
        assets_path=assets,
        cache_dir=cache,
    ...
    assert loaded.doc_kind == DocumentKind.asset
